...
Event OnUnequipped(Actor akActor)				
	...
	if !akActor.IsEquipped(deviceRendered) && UI.IsMenuOpen("InventoryMenu")
		libs.Log("Not proceeding with full logic")
		return
	EndIf
	if akActor == libs.PlayerRef
				...
				if npc != none && npc.GetItemCount(deviceRendered)!=0
					...
					if deviceKey && libs.PlayerRef.GetItemCount(deviceKey) >= NumberOfKeysNeeded
						...
						If !IsUnEquipDeviceConflict(npc)
							...
							if npc.GetActorBase().IsUnique() && (deviceRendered.HasKeyword(libs.zad_DeviousSuit) && !npc.WornHasKeyword(libs.zad_DeviousHeavyBondage)) || (deviceRendered.HasKeyword(libs.zad_DeviousHeavyBondage) && !npc.WornHasKeyword(libs.zad_DeviousSuit))
								Outfit OriginalOutfit = StorageUtil.GetFormValue(npc.GetActorBase(), "zad_OriginalOutfit") As Outfit
								If OriginalOutfit
									npc.SetOutfit(OriginalOutfit, false)
								EndIf
								StorageUtil.UnSetFormValue(npc.GetActorBase(), "zad_OriginalOutfit")
							endIf
						EndIf
